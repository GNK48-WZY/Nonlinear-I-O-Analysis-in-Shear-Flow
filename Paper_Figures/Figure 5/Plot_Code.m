...
delta_list = logspace(-6, 0, 200);
    ...
    h2 = loglog(p_list, local_Gamma_theorem .*local_u_tau_Lp + local_beta_theorem_p, '-.', 'LineWidth',1.5,'Color', colors(2, :), 'Marker',markers{2});
    h3 = loglog(p_list, y_tau_Linf.*ones(size(p_list)),'-r','Marker',markers{3}, 'LineWidth',1.5,'Color', colors(3, :));
    h4 = loglog(p_list, (local_Gamma_theorem .* u_tau_Linf + beta_theorem_inf).*ones(size(p_list)),'-k', 'LineWidth',1.5,'Color', colors(4, :),'Marker',markers{4});
